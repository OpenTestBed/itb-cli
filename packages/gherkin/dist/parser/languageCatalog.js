...
export function languageDecl(manifest) {
...
export function checkDialectImplementation(manifest) {
 ...
 *  apply as before). */
export function checkBaseCompatibility(core, manifest) {
...
export function queryDialectUrls() {
    try {
...
export function getStoredDialectUrls() {
    try {
...
export function addStoredDialectUrl(url) {
...
export async function loadRemoteComponent(baseUrl) {
    try {
        ...
        if (!mres?.ok)
            return null;
        const manifest = yaml.load(await mres.text());
        if (!manifest?.id)
            return null;
        let extension = null;
        ...
        if (eres.ok)
            extension = yaml.load(await eres.text());
        const scriptlets = [];
...
export async function loadComponentManifest(componentId) {
    try {
...
export async function loadComponentExtension(componentId, languageFile) {
    try {
...
export async function loadAllComponents(core) {
    if (!core) {
        try {
 ...
 */
export function mergeCatalog(core, components) {
...
export async function checkComponentHealth(manifest, endpointOverride) {
