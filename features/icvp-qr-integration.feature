Feature: Track 2: System Utilizes and Validates HCERT: ICVP
  ...
  Scenario: tc-icvp-001 QR -> decode -> transform -> ICVP IPS conformance
    ...
    # .tgz URL, so it loads exactly this build (do NOT also load https://smart.who.int/icvp
    # or the two same-canonical copies will conflict).
    # host.docker.internal:10005 = the package server's host-published port. The
